# python3 -m venv venv
# Elaborado por Fabian Fagua Marzo 2026
...
design_name = 'Estrella_Truncada_Circular'
nozzle_temp = 190
bed_temp =  60
print_speed = 1000
fan_percent = 100
...
numero_lados = 301 # numero de lados del poligono
...
ini_anuglo = 45 # angulo inicial de la silueta
radio_silueta = radius # radio de la siulueta
delta_ang =[]
radio_ang = []
for i in range(layers):
  #delta_ang.append(i*2*ini_anuglo/layers+ini_anuglo)
  delta_ang.append(i*2*ini_anuglo/layers-ini_anuglo)
  radio_ang.append(cos(radians(delta_ang[i])))
...
for layer in range(int(layers)):
    ...
    for i in range(len(angle_fractions2)):
        ...
        ## error steps.append(fc.polar_to_point(centre, radius*(radial_fractions2[i]+cos(delta_ang[layer])), tau*(angle_fractions2[i]+layer*fraccion/10)))
        steps.append(fc.polar_to_point(centre, radius*(radial_fractions2[i]+cos(radians(delta_ang[layer]))), tau*(angle_fractions2[i]+layer*fraccion/10)))
